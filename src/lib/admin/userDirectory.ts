...
export type AdminUserDirectoryAccessError = {
  error: string;
  code?: "unauthorized" | "admin_denied" | "admin_pending" | "admin_missing_domain";
};
...
export async function getAdminUserDirectory(): Promise<AdminUserDirectory> {
  ...
  if (!session?.access_token) {
    const error = new Error("Please sign in again.") as Error & { status?: number; code?: string };
    error.status = 401;
  ...
  if (!response.ok) {
    const message = payload && "error" in payload ? payload.error : "Failed to load user directory.";

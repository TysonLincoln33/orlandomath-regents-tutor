...
export default function PrintableSectionContent({
  ...
  return (
    <section className="chapter-print-section section-print-section">
      ...
      {data.lesson && printMode !== "answerKey" ? (
        ...
        {data.questions.map((question, idx) => {
          ...
          return (
            <div key={question.id} className="section-print-question bg-white rounded-xl shadow p-6">
              ...
              {printMode === "answerKey" ? (
                <div className="section-print-answer-key-bubbles mt-4" aria-label={`Question ${idx + 1} answer key`}>
                  {question.choices.map((_, choiceIndex) => (
                    <span
                      key={choiceIndex}
                      className={`section-print-answer-key-bubble ${choiceIndex === question.answerIndex ? "section-print-answer-key-bubble--correct" : ""}`}
                      aria-hidden="true"
                    />
                  ))}
                </div>

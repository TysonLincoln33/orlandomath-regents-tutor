...
export default function MasterPage() {
  ...
  const [expandedOversightGroupIds, setExpandedOversightGroupIds] = useState<string[]>([]);
  ...
  const [mExpandedGroupIds, setMExpandedGroupIds] = useState<string[]>([]);
  ...
  const toggleOversightGroup = (groupId: string) => setExpandedOversightGroupIds((prev) => prev.includes(groupId) ? prev.filter((id) => id !== groupId) : [...prev, groupId]);
  const toggleMGroup = (groupId: string) => setMExpandedGroupIds((prev) => prev.includes(groupId) ? prev.filter((id) => id !== groupId) : [...prev, groupId]);
  ...
  return <main className="min-h-screen bg-slate-950 p-6 text-white"><div className="mx-auto max-w-7xl">
    ...
    <p className="mb-6 text-sm text-white/80">Global Regents Algebra 1 overview + interactive classroom management.</p>

    {/* existing overview + assignments mostly unchanged */}
    <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
          <aside className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <h2 className="mb-3 text-xl font-bold">All Users</h2>
            {users.length === 0 ? <p className="text-white/80">No Algebra 1 users found yet.</p> : <ul className="space-y-3">{users.map((u) => <li key={u.user_id} className="rounded-xl border border-white/10 p-3"><p className="font-semibold">{displayName(u)}</p><p className="text-xs text-white/80">{u.email ?? 'No email'}</p><p className="text-xs text-white/80">Last activity: {formatDateTime(u.last_activity_at, 'No activity yet')}</p><button onClick={() => void onSelectUser(u.user_id)} className="mt-2 rounded-md bg-indigo-500 px-3 py-1 text-sm">View Progress</button></li>)}</ul>}
          </aside>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            ].map(([k, v]) => <div key={String(k)} className="rounded-lg border border-white/10 bg-black/20 p-3"><p className="text-xs text-white/80">{k}</p><p className="text-xl font-bold">{v}</p></div>)}</div>

            <div className="mt-6">
              <h2 className="text-xl font-bold">{selectedUser ? `${displayName(selectedUser)} Progress` : 'Recent Attempts'}</h2>
              {selectedUser && <p className="mb-3 text-sm text-white/80">{selectedUser.email ?? 'No email'} · Completion {Math.round(Number(selectedUser.completion_percent ?? 0))}% · Accuracy {selectedUser.accuracy_percent == null ? '—' : `${Math.round(Number(selectedUser.accuracy_percent))}%`} · Attempts {Number(selectedUser.attempts_count ?? 0)} · Correct {Number(selectedUser.correct_count ?? 0)}</p>}
              {panelAttempts.length === 0 ? <p className="text-white/80">No recent attempts available yet.</p> : <ul className="space-y-2">{panelAttempts.map((a, i) => <li key={`${a.user_id}-${a.question_id}-${a.attempted_at ?? i}`} className="rounded-lg border border-white/10 p-3"><p className="font-medium">{displayName(a)} · {a.section_title}</p><p className="text-sm text-white/80">Question: {a.question_id ?? 'Unknown'} · {formatDateTime(a.attempted_at)}</p><span className={`mt-1 inline-block rounded-full px-2 py-0.5 text-xs ${a.correct ? 'bg-emerald-500/20 text-emerald-200' : 'bg-rose-500/20 text-rose-200'}`}>{a.correct ? 'Correct' : 'Incorrect'}</span></li>)}</ul>}
            </div>
          </section>
        </div>
        ...
        <section className="mt-8 rounded-2xl border-2 border-indigo-400/40 bg-white/5 p-5" aria-label="Master Assignment Oversight">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-white/80">Read-only assignment oversight for Regents Algebra 1 across all classrooms.</p>
            </div>
          </div>

          {assignmentsLoading ? <p className="text-sm text-white/80">Loading assignments…</p> : null}
          ...
          {!assignmentsLoading && !assignmentError ? (
            <>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                <input value={assignmentSearch} onChange={(event) => setAssignmentSearch(event.target.value)} placeholder="Search title, classroom, teacher, or section" className="rounded-lg border border-white/20 bg-black/20 px-3 py-2 text-sm text-white placeholder:text-white/70" />
              ...
              <div className="mt-4 overflow-x-auto">
                {assignmentGroups.length === 0 ? <p className="text-white/80">No assignments yet.</p> : <table className="min-w-full border-collapse text-sm"><thead><tr className="border-b border-white/10 text-left text-white/80"><th className="px-2 py-2">Teacher</th><th className="px-2 py-2">Classroom</th><th className="px-2 py-2">Assignment group</th><th className="px-2 py-2">Sections</th><th className="px-2 py-2">Due date</th><th className="px-2 py-2">Created</th><th className="px-2 py-2">Updated</th><th className="px-2 py-2">Status</th><th className="px-2 py-2">Recipients</th><th className="px-2 py-2">Actions</th></tr></thead><tbody>{assignmentGroups.map((group) => { const isExpanded = expandedOversightGroupIds.includes(group.id); return <Fragment key={group.id}><tr className="border-b border-white/5 align-top"><td className="px-2 py-2"><p>{group.teacherName ?? 'Unknown teacher'}</p><p className="text-xs text-white/80">{group.teacherEmail ?? 'No email'}</p></td><td className="px-2 py-2">{group.classroomName ?? group.classroomId}</td><td className="px-2 py-2"><p className="font-semibold">{group.title}</p><p className="text-xs text-white/80">{group.description || 'No description'}</p></td><td className="px-2 py-2"><p className="mb-1 text-xs text-white/80">{group.sectionIds.length} section{group.sectionIds.length === 1 ? '' : 's'}</p><div className="flex max-w-xs flex-wrap gap-1">{group.sectionIds.map((sectionId) => <span key={`${group.id}-${sectionId ?? 'none'}`} className="rounded-full border border-indigo-300/30 bg-indigo-400/10 px-2 py-0.5 text-xs">{getShortSectionLabel(sectionId)}</span>)}</div></td><td className="px-2 py-2">{formatCalendarDate(group.dueDate)}</td><td className="px-2 py-2">{formatDateTimeRange(group.createdAtStart, group.createdAtEnd)}</td><td className="px-2 py-2">{formatDateTime(group.updatedAtLatest)}</td><td className="px-2 py-2">{group.isArchived ? 'Archived' : 'Active'}</td><td className="px-2 py-2">{group.recipientCount} total · {group.completedCount} complete · {group.incompleteCount} incomplete · {group.excusedCount} excused</td><td className="px-2 py-2"><button type="button" onClick={() => toggleOversightGroup(group.id)} className="rounded bg-slate-600 px-2 py-1 text-xs">{isExpanded ? 'Hide sections' : 'View sections'}</button></td></tr>{isExpanded ? group.assignments.map((assignment) => <tr key={assignment.assignment_id} className="border-b border-white/5 bg-black/20 align-top text-xs"><td className="px-2 py-2" colSpan={3}>{assignment.title}</td><td className="px-2 py-2">{getSectionLabel(assignment.section_id)}</td><td className="px-2 py-2">{formatCalendarDate(assignment.due_date)}</td><td className="px-2 py-2">{formatDateTime(assignment.created_at)}</td><td className="px-2 py-2">{formatDateTime(assignment.updated_at)}</td><td className="px-2 py-2">{assignment.archived_at ? 'Archived' : 'Active'}</td><td className="px-2 py-2" colSpan={2}>{Number(assignment.recipient_count ?? 0)} total · {Number(assignment.completed_count ?? 0)} complete · {Number(assignment.incomplete_count ?? 0)} incomplete · {Number(assignment.excused_count ?? 0)} excused</td></tr>) : null}</Fragment>; })}</tbody></table>}
              </div>
            </>
          ) : null}
        </section>
    ...
    <section className="mt-8 rounded-2xl border-2 border-cyan-400/40 bg-white/5 p-5" aria-label="Master Assignment Management">
      ...
      <div className="mt-3 rounded border border-white/10 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-white/80">Select any mix of chapters and individual sections. One assignment row will be created per resolved section.</p>
          </div>
      ...
      <div className="mt-4 space-y-3">{mAssignmentGroups.map((group) => { const isExpanded = mExpandedGroupIds.includes(group.id); return <div key={group.id} className="rounded border border-white/10 bg-black/10 p-3 text-sm"><div className="flex flex-wrap justify-between gap-3"><div className="min-w-0"><p><b>{group.title}</b> · {group.classroomName ?? group.classroomId} · {formatCalendarDate(group.dueDate)} · {group.isArchived ? 'Archived' : 'Active'}</p><p className="text-xs text-white/80">{group.description || 'No description'} · Created {formatDateTimeRange(group.createdAtStart, group.createdAtEnd)} · Updated {formatDateTime(group.updatedAtLatest)}</p><p className="mt-1 text-xs text-white/80">{group.recipientCount} recipients · {group.completedCount} complete · {group.incompleteCount} incomplete · {group.excusedCount} excused</p><div className="mt-2 flex flex-wrap gap-1"><span className="rounded-full border border-cyan-300/40 bg-cyan-400/10 px-2 py-0.5 text-xs">{group.sectionIds.length} section{group.sectionIds.length === 1 ? '' : 's'}</span>{group.sectionIds.map((sectionId) => <span key={`${group.id}-${sectionId ?? 'none'}`} className="rounded-full border border-white/15 bg-white/10 px-2 py-0.5 text-xs">{getShortSectionLabel(sectionId)}</span>)}</div></div><button type="button" onClick={() => toggleMGroup(group.id)} className="self-start rounded bg-slate-600 px-2 py-1 text-xs">{isExpanded ? 'Hide section assignments' : 'View sections'}</button></div>{isExpanded ? <div className="mt-3 space-y-2 border-t border-white/10 pt-3">{group.assignments.map((a) => { const isEditing = mEditingAssignmentId === a.id; const isArchived = Boolean(a.archived_at); return <div key={a.id} className="rounded border border-white/10 p-2"><div className="flex flex-wrap justify-between gap-2"><div><b>{getSectionLabel(a.section_id)}</b><p className="text-xs text-white/80">{formatCalendarDate(a.due_date)} · {isArchived ? 'Archived' : 'Active'} · {Number(a.recipient_count ?? 0)} total · {Number(a.completed_count ?? 0)} complete · {Number(a.incomplete_count ?? 0)} incomplete · {Number(a.excused_count ?? 0)} excused</p></div><div className="flex flex-wrap gap-2">{isEditing ? <><button onClick={async()=>{ try { if (!mEditTitle.trim()) { setMMsg('Assignment title is required.'); return; } await updateMasterAssignment(a.id,{title:mEditTitle,description:mEditDescription,due_date:mEditDueDate || null}); cancelEditMasterAssignment(); await refreshMasterAssignmentPanels(); setMMsg('Assignment updated.'); } catch (e) { setMMsg(getErrorMessage(e, 'Failed to update assignment.')); } }} className="rounded bg-emerald-600 px-2">Save</button><button onClick={cancelEditMasterAssignment} className="rounded bg-slate-600 px-2">Cancel</button></> : <button onClick={()=>beginEditMasterAssignment(a)} className="rounded bg-indigo-600 px-2">Edit</button>}<button onClick={async()=>{ try { if (isArchived) { setMMsg('Assignment is already archived.'); return; } await archiveMasterAssignment(a.id); await refreshMasterAssignmentPanels(); setMMsg('Assignment archived.'); } catch (e) { setMMsg(getErrorMessage(e, 'Failed to archive assignment.')); } }} className="rounded bg-rose-600 px-2 disabled:opacity-60" disabled={isArchived}>{isArchived ? 'Archived' : 'Archive'}</button><button onClick={() => void loadMasterRecipients(a.id)} className="rounded bg-slate-600 px-2">View Recipients</button></div></div>{isEditing ? <div className="mt-2 grid gap-2 md:grid-cols-3"><input value={mEditTitle} onChange={(e)=>setMEditTitle(e.target.value)} placeholder="Assignment title" className="rounded bg-black/20 px-2 py-1" /><input value={mEditDescription} onChange={(e)=>setMEditDescription(e.target.value)} placeholder="Description" className="rounded bg-black/20 px-2 py-1" /><input type="date" value={mEditDueDate} onChange={(e)=>setMEditDueDate(e.target.value)} className="rounded bg-black/20 px-2 py-1" /></div> : null}</div>; })}</div> : null}</div>; })}</div>
      {mRecipientsLoading ? <p className="mt-3 text-sm text-white/80">Loading recipients…</p> : null}
    ...
    <section className="mt-8 rounded-2xl border-2 border-emerald-400/40 bg-white/5 p-5" aria-label="Master Classroom Management">
      ...
      <p className="mb-4 text-sm text-white/80">Global classroom and student roster management.</p>
      ...
      <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
        ...
        <div className="rounded-xl border border-white/10 p-3">
          {!selectedClassroom ? <p>Select a classroom.</p> : <>
            ...
            <p className="mb-3 text-sm text-white/80">Teacher: {selectedClassroom.teacher_name ?? 'Unknown'} ({selectedClassroom.teacher_email ?? 'No email'})</p>
            ...
            <div className="mb-3 max-h-40 space-y-1 overflow-y-auto rounded-lg border border-white/10 p-2 text-sm">{availableSearchResults.length === 0 ? <p className="text-white/80">No matching students.</p> : availableSearchResults.map((s) => <label key={s.id} className="flex cursor-pointer items-center gap-2"><input type="checkbox" checked={selectedSearchStudentIds.includes(s.id)} onChange={() => setSelectedSearchStudentIds((prev) => prev.includes(s.id) ? prev.filter((id) => id !== s.id) : [...prev, s.id])} /><span>{s.full_name ?? s.email} <span className="text-xs text-white/80">({s.email ?? 'No email'})</span></span></label>)}</div>

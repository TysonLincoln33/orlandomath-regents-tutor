...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (!quickClass) {
      return NextResponse.json({ studentUserId, chapterId, unassignedRecipientCount: 0 });
    }
    ...
    const { data: unassignedRecipients, error: unassignError } = await ctx.adminClient
      .from("assignment_recipients")
      .update({ status: "unassigned" })
      .eq("classroom_id", quickClass.id)
      .eq("user_id", studentUserId)
      .in("assignment_id", assignmentIds)
      .in("status", ["assigned", "completed", "excused"])
      .select("id");

    if (unassignError) {
      throw new AdminClassroomManagementApiError(
        unassignError.message || "Failed to unassign Quick Assign chapter.",
        500,
      );
    }
    ...
    return NextResponse.json({
      studentUserId,
      chapterId,
      unassignedRecipientCount: unassignedRecipients?.length ?? 0,
    });
  } catch (error) {
    console.error("admin quick assign chapter unassign route error", error);

...
export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const { data, error } = await adminClient
      .from("profiles")
      .update({ role: "student", approval_status: "denied" })
      .eq("id", profileId)
    ...
    if (error) {
      throw new Error(error.message || "Failed to deny administrator request.");
    }
    ...
    return NextResponse.json({
      message: "Administrator request denied.",
      profile: {
        id: data.id,
        role: data.role,
    ...
    console.error("admin approval deny route error", error);

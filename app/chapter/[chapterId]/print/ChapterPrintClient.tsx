...
import { PrintableChapterAnswerKey } from "@/components/print/PrintableAnswerKey";
      ...
      <div className="section-print-toolbar no-print rounded-xl border border-slate-200 bg-white p-4 shadow">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={printAnswerKey} className="rounded-lg bg-slate-800 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-900">Print Chapter Answer Key</button>
          </div>
        </div>
      </div>

      {printMode === "answerKey" ? (
        <PrintableChapterAnswerKey chapterNumber={data.chapterNumber} sections={data.sections} />
      ) : (
        <>

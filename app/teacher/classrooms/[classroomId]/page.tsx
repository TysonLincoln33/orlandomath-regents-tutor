...
import {
  ...
  type TeacherAssignmentRecipientsResult,
} from "@/lib/classrooms/getTeacherAssignmentRecipients";
import {
  updateAssignmentRecipientStatus,
  type AssignmentRecipientStatusAction,
} from "@/lib/classrooms/updateAssignmentRecipientStatus";
import {
...
export default function ClassroomDetailPage({ params }: PageProps) {
  ...
  const [selectedAssignmentId, setSelectedAssignmentId] = useState<
    string | null
  >(null);
  const [assignmentRecipientDetail, setAssignmentRecipientDetail] =
    useState<TeacherAssignmentRecipientsResult | null>(null);
  const [assignmentRecipientLoading, setAssignmentRecipientLoading] =
    useState(false);
  const [assignmentRecipientError, setAssignmentRecipientError] = useState<
    string | null
  >(null);
  const [updatingRecipientUserId, setUpdatingRecipientUserId] = useState<
    string | null
  >(null);
  ...
  const loadAssignmentRecipientDetail = async (assignmentId: string) => {
    if (!classroomId) return;

    try {
      setAssignmentRecipientLoading(true);
      setAssignmentRecipientError(null);
      ...
      setAssignmentRecipientDetail(detail);
    } catch (err) {
      console.error(err);
      setAssignmentRecipientDetail(null);
      setAssignmentRecipientError(
        getErrorMessage(err, "Failed to load assignment recipients."),
      );
    } finally {
      setAssignmentRecipientLoading(false);
    }
  };

  const handleOpenAssignmentDetail = async (
    ...
    setAssignmentRecipientError(null);
    cancelEditingAssignment();
    await loadAssignmentRecipientDetail(assignment.id);
  };
  ...
  const handleViewRecipientProgress = async (
    recipient: TeacherAssignmentRecipient,
  ) => {
    await handleViewStudentProgress({
      id: `${classroomId}:${recipient.userId}`,
  ...
  const handleUpdateRecipientStatus = async (
    ...
    if (!classroomId || !selectedAssignmentId) return;
    ...
    try {
      ...
      const [detail, refreshedAssignments] = await Promise.all([
        getTeacherAssignmentRecipients(classroomId, selectedAssignmentId),
        getClassroomAssignments(classroomId),
      ]);
      ...
      setAssignmentMessage(
        action === "excuse"
          ...
          : "Recipient un-excused.",
      );
    } catch (err) {
      console.error(err);
      setAssignmentRecipientError(
        getErrorMessage(err, "Failed to update assignment recipient."),
      );
    } finally {
      setUpdatingRecipientUserId(null);
    }
            ...
            <section className="xl:col-span-1 bg-white rounded-xl shadow border border-gray-200 p-6">
              ...
              {selectedAssignment ? (
                <AssignmentRecipientDetailPanel
                  ...
                  detail={assignmentRecipientDetail}
                  loading={assignmentRecipientLoading}
                  error={assignmentRecipientError}
                  updatingRecipientUserId={updatingRecipientUserId}
                  onBack={handleBackToAssignmentList}
                  onRetry={() =>
                    loadAssignmentRecipientDetail(selectedAssignment.id)
                  }
                  onStatusAction={handleUpdateRecipientStatus}
                  onViewStudentProgress={handleViewRecipientProgress}
                />
              ) : (
                <>
...
function AssignmentRecipientDetailPanel({
  ...
  assignment: ClassroomAssignment;
  detail: TeacherAssignmentRecipientsResult | null;
  loading: boolean;
  error: string | null;
  updatingRecipientUserId: string | null;
  onBack: () => void;
  onRetry: () => void;
  onStatusAction: (
    recipient: TeacherAssignmentRecipient,
  ...
  onViewStudentProgress: (recipient: TeacherAssignmentRecipient) => void;
}) {
  ...
  return (
    <div className="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <button
            type="button"
            onClick={onBack}
            className="mb-3 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-100"
          >
            Back to assignments
          </button>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <AssignmentCountPill
          label="Completed"
          value={summary?.completedCount ?? assignment.completed_count ?? 0}
        />
        <AssignmentCountPill
          label="Incomplete"
          value={summary?.incompleteCount ?? assignment.incomplete_count ?? 0}
        />
        <AssignmentCountPill
          label="Excused"
          value={summary?.excusedCount ?? assignment.excused_count ?? 0}
        />
        <AssignmentCountPill
...
function RecipientStatusBadge({
  ...
  const label = recipient.isExcused
    ? "Excused"
    : recipient.isComplete
      ? "Completed"
      : recipient.status.replace("_", " ");
  const className = recipient.isExcused
  ...
  return (
    <span
      className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold capitalize ${className}`}
    >
      {label}
    </span>
  );
